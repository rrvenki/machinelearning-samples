...
Namespace Model
    Public Class ModelEvaluator
        ...
        Public Async Function Evaluate() As Task
            ...
            Dim model = Await PredictionModel.ReadAsync(Of ImageNetData, ImageNetPrediction)(modelLocation)

            ' Get Predictions
            Dim predictions = GetPredictions(dataLocation, imagesFolder, model).ToArray()
            ShowMetrics(dataLocation, model)
        End Function

        Protected Iterator Function GetPredictions(testLocation As String, imagesFolder As String, model As PredictionModel(Of ImageNetData, ImageNetPrediction)) As IEnumerable(Of ImageNetPrediction)
        ...
        Protected Sub ShowMetrics(testLocation As String, model As PredictionModel(Of ImageNetData, ImageNetPrediction))
            Dim evaluator = New ClassificationEvaluator()
            Dim testDataSource = (New TextLoader(testLocation)).CreateFrom(Of ImageData.ImageNetData)()
